...
(function (Canvas) {
    ...
    function drawTriangles(_position) {
        ...
        let nTriangles = 10000; // 10.000 Dreiecke
        ...
        for (let drawn = 0; drawn < nTriangles; drawn++) {
            ...
            let x = Math.random() * _position.x; // Zufällige x-Position des Dreiecks
            let y = Math.random() * _position.y; // Zufällige y-Position des Dreiecks
            // Erstellung eines linearen Farbverlaufs für jedes Dreieck
            let gradient = crc2.createLinearGradient(x, y, x + 20, y + 20);
    ...
    function drawCircles(_position) {
        ...
        let nCircles = 80; // Anzahl der zu zeichnenden Kreise
        crc2.save(); // Speichern des aktuellen Zeichenzustands
        for (let drawn = 0; drawn < nCircles; drawn++) {
            crc2.save(); // Speichern des aktuellen Zeichenzustands für jeden einzelnen Kreis
            let x = Math.random() * _position.x; // Zufällige x-Position des Kreises
            let y = Math.random() * _position.y; // Zufällige y-Position des Kreises
            let radius = Math.random() * 50; // Zufälliger Radius für den Kreis
